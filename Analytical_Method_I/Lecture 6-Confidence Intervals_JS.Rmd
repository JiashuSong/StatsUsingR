...
similar: yes. because it is random sampling with same sample size. 
...
# 5.Does your confidence interval capture the true average size of houses in Ames? 
  ...
  Yes, 1499.69 falls into the confiendce interval[1419.500, 1645.267]
...
46 out of 50 C.L. include the true population mean.That is 92%. If not equals to the confidence level.Because these are random sampling processing, the C.L does not equal to accuracy rate.
```{r}
par(mfrow = c(1, 1))
plot_ci(lower_vector, upper_vector, mean(population))
...
dfsub<- subset(df , lower_vector <meanp & upper_vector > meanp)
...
the percentage decreased to 86% (43/50) with lower C.L.(90%)
```{r}
...
df90 = data.frame(lower_vector90, upper_vector90)

dfsub90<- subset(df90 , lower_vector90 <meanp & upper_vector90 > meanp)
