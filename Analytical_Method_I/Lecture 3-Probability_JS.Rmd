...
# ref:https://rpubs.com/uplotnik/468061
...
#    Pr(shot1 =H) =.45
#    Pr(shot2= h given Shot1 = H)  > .45 ??  eg. 0.60  if Kobe has a hot hand. # bayesian?
...
##   of 50% we type,
      ...
      outcomes <- c("H", "M")
      sim_basket <- sample(outcomes, size = 1, replace = TRUE, prob = c(.45, 0.55))
...
## of baskets in 133 shots?
       ...
       #answer: This distributionis right skewed. typical streak length is 0, the longest is 6, 
       #        even longer than Kobe's record. 
...
```{r}
sim_basket2 <- sample(outcomes, size = 133, replace = TRUE, prob = c(.436, 0.564))
table(sim_basket2)
calc_streak(sim_basket2)
```
       
       barplot(table(calc_streak(sim_basket2)))
       
       #Answer: expect similar distribution, not exactly the same due to the sim_basket data changed
       #        with seeding, the imulated data won't change so the result will be exactly same. 
...
#       do you have evidence that the hot hand model fits Kobe's shooting patterns? Explain
       ...
       # Answer: Don't see stronge evidence to support "hat hand theory". From my repeated simulations using 
       #        sim_basket2 data, The longest streak from simulated shooter data
       #        could have longer streak or shorter streak than Kobe's record. 
       #        running the simulated testing for many times, the distribution/trend may tell us more.  
